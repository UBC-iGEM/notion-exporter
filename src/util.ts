...
export async function getTitles(
    pageId: string,
): Promise<{ databaseTitle: string; pageTitle: string }> {
...
export async function getPageMetadata(pageId: string): Promise<string> {
    const page = (await notion.getPage({ pageId })) as PageObjectResponse;
    const metadata = Object.entries(page)
        .map(([key, value]) => {
...
export async function parseDatabases({
    ...
    await Promise.all(
        databaseIds.map(async (databaseId) => parseDatabase({ databaseId })),
    );
}
    ...
    await Promise.all(
        pageIds.map(async (pageId) => {
            ...
            const codeFenceStart =
                pageContent.indexOf(codeFence) + codeFence.length;
            const codeFenceEnd =
                pageContent.indexOf(codeFence, codeFenceStart) +
                codeFence.length;
...
export async function parsePage({
    ...
    let response = await converter.convert({
        block,
        databaseTitle,
        pageTitle,
        parentType,
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        if (component.delimiterState(blocks[i])) {
            ...
            while (
                component.delimiterState(blocks[i]) !== false &&
                i < blocks.length
            ) {
            ...
            const componentTag = await component.ingest(
                componentType,
                componentBlocks,
            );
    ...
    if (
        converter.lineBreakTypes.includes(type) &&
        !converter.lineBreakTypes.includes(parentType)
    ) {
        content.value = content.value.concat("\n");
    }
