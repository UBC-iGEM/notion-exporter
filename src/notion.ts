...
const notionClients: Client[] = notionKeys.map(
    (notionKey) => new Client({ auth: notionKey }),
);
...
export async function queryDatabase({
    ...
    databaseId: string;
    opts?: Partial<QueryDatabaseParameters>;
...
function retryAfter(error: Error): void {
    if (error instanceof APIResponseError) {
        ...
        console.log(
            `\x1b[1m\x1b[33mRate limited - retry after: ${minutes}m ${seconds}s\x1b[0m`,
        );
    }
}
