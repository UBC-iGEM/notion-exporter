...
export async function convert({
    ...
    const bulletedListItemContent =
        type !== "bulleted_list_item" && parentType === "bulleted_list_item";
    const numberedListItemContent =
        type !== "numbered_list_item" && parentType === "numbered_list_item";
    ...
    // Apply newline
    if (
        !omitTypes.includes(type) &&
        !bulletedListItemContent &&
        !numberedListItemContent &&
        !toggleContent
    ) {
        if (
            lineBreakTypes.includes(type) ||
            lineBreakTypes.includes(parentType) ||
            response === ""
        ) {
            // Line break
            response = response.concat("\n");
...
async function formatContent(
    contentBlocks: RichTextItemResponse[],
): Promise<string> {
    let response = "";
    ...
    for (const contentBlock of contentBlocks) {
        ...
        for (const [annotation, enabled] of Object.entries(
...
async function citeKeyFromPageIfPresent(
    page: PageObjectResponse,
): Promise<string | null> {
    const pageProperties = page.properties;
    if (
        pageProperties.prototype &&
        !pageProperties.prototype.hasOwnProperty("Citation Key")
    ) {
        return null;
    }
    ...
    if (citationKey.rich_text.length === 0) {
        console.error(
            `Citation Key property is present but empty on page @ ${page.url}`,
        );
        return null;
    }
...
async function checkForIntraWikiLink(pageId: string): Promise<string> {
    ...
    if (databaseTitle) {
        const cleanDbTitle = databaseTitle
            .toLowerCase()
            .replaceAll(" ", "-")
            .replaceAll("+", "")
            .replaceAll(":", "");
...
async function bulleted_list_item(
    ...
    if (
        converterSettings.parentType !== "bulleted_list_item" ||
        (converterSettings.parentType === "bulleted_list_item" &&
            converterSettings.index === 0)
    ) {
    ...
    if (
        converterSettings.parentType === "bulleted_list_item" &&
        converterSettings.index === converterSettings.lastIndex
    ) {
...
async function numbered_list_item(
    ...
    if (
        converterSettings.parentType !== "numbered_list_item" ||
        (converterSettings.parentType === "numbered_list_item" &&
            converterSettings.index === 0)
    ) {
        return `<ol>\n${indent(`<li>${text}</li>`, converterSettings.indentation + 1)}`;
